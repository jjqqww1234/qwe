...
type
  ...
  TChgAttr = record
    ...
    min:  byte;
    //DC,MC,SC의 min/max  AC,MAC인경우 MakeWord(min,max)값임
    max:  byte;
  end;
...
{$else}
  ...
  //미르2
  TStdItem = record
    Name:    string[20];        // 아이템 이름 (천하제일검)
    StdMode: byte;
    Shape:   byte;              // 형태별 이름 (철검)
    Weight:  byte;              // 무게
    AniCount: byte;
    // 1보다 크면 애니메이션 되는 아이템 (다른 용도로 많이 쓰임)
    SpecialPwr: shortint;          // +이면 생물공격+능력, -이면 언데드공격+
    //1~10 강도
    //-50~-1 언데드 능력치 향상
    //-100~-51 언데드 능력치 감소
    ItemDesc: byte;
    //$01 IDC_UNIDENTIFIED  (아이댄티파이 안 된 것, 클라이언트에서만 사용됨)
    //$02 IDC_UNABLETAKEOFF (손에서 떨어지지 않음, 미지수 사용 가능)
    //$04 IDC_NEVERTAKEOFF  (손에서 떨어지지 않음, 미지수 사용 불가능)
  ...
  TUserItem = packed record
    MakeIndex: integer;
    //서버에서의 아이템 인덱스(만들어 질때 인덱스 매겨짐, 중복가능)
    Index:     word;          //표준아이템의 인덱스  0:없음, 1부터 시작함..
    Dura:      word;
    DuraMax:   word;          //변경된 내구성 최대값
    Desc:      array[0..13] of byte;
  ...
  TAbility = record
    ...
    WearWeight: byte;
    MaxWearWeight: byte;
    //검을 제외한 착용 가능한 아이템의 무게 (초과하면 굉장히 느리다 2-3배느림)
    HandWeight: byte;
    MaxHandWeight: byte;
    //들 수 있는 검의 무게 (무게를 초과하면 굉장히 느리게 공격한다 2-3배느림)
  end;
...
{$endif}//미르2
  ...
  TDefMagic = record
    MagicId:  word;
    MagicName: string[14];       //칸 늘릴것 12 -> 14
    EffectType: byte;
    Effect:   byte;
    Spell:    word;
    MinPower: word;
  ...
  TUserMagic = record
    ...
    MagicId:  word;
    //Magic Index 저장. 유니크해야하며, 변동되면 안됨, 항상 0보다 크다.
    Level:    byte;
...
{$endif}
  ...
  //연인의 플래그가 TRUE인지 검사(연인정보를 찾을 수 없으면 FALSE 리턴)
  ...
  QI_CHECKCHILDMOB  = 150;    // 소환중인 몬스터 이름으로 체크
  ...
  QA_BATCHMOVE     = 55;
  QA_PLAYDICE      = 56;
  //PLAYDICE 2 @diceresult //2개의 주사위를 굴린다. 그후 @diceresult 세션으로 간다
  //6-11
  QA_ADDNAMELIST   = 57;
  ...
  //파라메터,  최소, 최대  예) 401 450  401에서 450번까지 랜덤으로 설정
  QA_SETDAILYQUEST = 61;
  ...
  // 그룹 멤버들 중에 현재 맵에 있는 멤버들만 특정 맵으로 이동시킴.
  QA_RECALLMAPGROUP = 108;
  // 그룹 멤버들 중에 특정 맵에 있는 멤버들만 현재 맵으로 이동시킴.
  ...
  MARKEY_DBSELLTYPE_SELL     = 1;    //판매중
  MARKEY_DBSELLTYPE_BUY      = 2;    //샀음
  MARKEY_DBSELLTYPE_CENCEL   = 3;    //취소
  MARKEY_DBSELLTYPE_GETPAY   = 4;    //금액회수
  MARKEY_DBSELLTYPE_READYSELL = 11;  //임시 판매중
  MARKEY_DBSELLTYPE_READYBUY = 12;   //임시 사는중
  MARKEY_DBSELLTYPE_READYCANCEL = 13;//임시 취소중
  MARKEY_DBSELLTYPE_READYGETPAY = 14;//임시 회수중
  MARKEY_DBSELLTYPE_DELETE   = 20;   //삭제
