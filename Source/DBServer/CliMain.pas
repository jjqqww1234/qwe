...
begin
  for i := 0 to UserList.Count - 1 do begin
    ...
    if puser.Socket = Socket then begin
      ...
      n   := Pos('*', str);                // 서버가 보낸 채크에 클라이언트의 응답
      if n > 0 then begin
        temp := Copy(str, 1, n - 1);
        str  := temp + Copy(str, n + 1, Length(str));
        puser.SendLength := 0;
      end;
      n := Pos('$', str);                // 클라이언트가 서버의 응답을 물음
      if n > 0 then begin
        temp := Copy(str, 1, n - 1);
        str  := temp + Copy(str, n + 1, Length(str));
...
begin
  SendSocket(Socket, '$');  //클라이언트가 보내는 채크에 대답
end;
               ...
               if OpenRd then begin
                  while TRUE do begin
                     if idx < FeeIndex.Count then begin
                        if GetRecord (idx, rcd) >= 0 then begin
                           if CheckValidDate (rcd) then //가능한 계정중에서
                              if (rcd.Block.AccountMode <> 2) and (rcd.Block.EnableDate <= LimitDate) then begin //제한일보다 적으면
