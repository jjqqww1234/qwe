...
type
  ...
  TUserInfo = record
    ...
    ConnectTime: longword;        // 처음 접속한 시간,
...
begin
  ...
  //certifystr이 로그인서버로 부터 받은 인증목록에 있는지 검사한다.
  if FrmIDSoc.GetAdmission(uid, pu.UserAddr, certify) then begin //승인이된 접속인지

    FrmIDSoc.ClearSelectedAdmission(certify); //재접속인경우 <캐릭선태했음> 초기화
...
begin
  ...
  if ok then begin
    ServerIndex := GetServerFromMap(map);  //멀티서버인경우 접속포트+1씩 된다.
