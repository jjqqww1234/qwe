...
type
  TFeeInfo = record
    ...
    GroupKey: string[30];   //PC방 이름
    OwnerName: string[10];
    AccountMode: byte;      //0: 무료, 1: 정액, 2:종량,
    DupCount: word;
    EntryDate: TDateTime;   //등록일
    NCount: integer;        //접수날짜
    EnableDate: TDateTime;  //사용가능 일자
    Memo: string[20];
  end;
...
begin
  ...
  if WriteRecord(integer(FeeIndex.Objects[index]), rcd, False) then begin
    Result := True;
    ///FeeIndex[index] := rcd.Key; <= 키는 바뀔 수 없다.
      {if flag then
