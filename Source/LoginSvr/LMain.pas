...
type
  TConnInfo = record
    ...
    Closing:  boolean; //true이면 중복접속으로 종료중. 30초후 완전종료
  end;
  ...
  TUserInfo = record
    ...
    PayMode:    byte;           //과금모드  0:체험판, 1:유료  2:무료이용자
    IDDay:      integer;
    IDHour:     integer;
    IPDay:      integer;
    IPHour:     integer;
    ...
    // 처음 접속한 시간, 다른 서버로 옮겼으며 옮긴 처음 시간
...
begin
  for i := 0 to CertList.Count - 1 do begin
    ...
    if (PTConnInfo(CertList[i]).Uid = uid) and
      ...
      PTConnInfo(CertList[i]).OpenTime := GetTickCount; //닫히기 시작한 시작
...
begin
  ...
  for i := 0 to CertList.Count - 1 do begin
    if PTConnInfo(CertList[i]).Certify = cert then begin
      if not PTConnInfo(CertList[i]).Closing then //해지당하지 않은 것만
        Result := True;
      break;
    end;
  end;
...
begin
  for i := 0 to ginfo.UserList.Count - 1 do begin
    if PTUserInfo(ginfo.UserList[i]).UserHandle = uhandle then begin
      ...
      if not p.SelServerOk then  //서버선택을 하지않은 채로 접속종료된 경우
...
begin
  ...
  if success = 1 then begin
    ...
    puser.SelServerOk := False; //서버 선택중 접속이 끊어질 수 있음.
...
begin
  ...
  if (puser.UserId <> '') and (svname <> '') and
    IsValidCertification(puser.Certification) then begin //Passwd를 통과했어야함.
    ...
    if (runaddr <> '') and (runport > 0) then begin
      ...
      puser.SelServerOk := True; //서버 선택이 성공적임.
      freemode := False;

      //SelChr서버(DBServer)에도 id+certificaion을 보내야 한다.
      //...+ uid + '/' +
      //PayMode : 0(무료체험판) 1(정식사용자)
      availabletype := 5;
      if puser.IDHour > 0 then
        availabletype := 2;
      if puser.IPHour > 0 then
        availabletype := 4;  //pc방은 -시간을 준다.
      ...
      if KoreanVersion then begin
        //공작, 봉황, 해태 서버 무료로 돌림
        if (svname = '공작서버') or (svname = 'BongWangServer') or
          (svname = '해태서버') then begin
          if Now < CalcDay(puser.AccountMakeDate, 20) then begin
            puser.PayMode := 2;     //무료사용자
            freemode      := True;  //끊기지 않게 하려고
          end;
        end;
      end;
        ...
        if idx >= 0 then begin
          if GetRecord(idx, ircd) >= 0 then begin
            ...
            begin //3분동안 막힌다.
              if passwd = ircd.Block.Uinfo.Password then begin
                ircd.Block.PassWdFail := 0;
                ircd.Block.Uinfo.Password := newpass; //새비밀번호로 변경
                success := 1; //성공
              end else begin
                ircd.Block.PasswdFail := ircd.Block.PassWdFail + 1;
                ircd.Block.PasswdFailTime := GetTickCount;
  ...
  try
    ...
    case msg.Ident of
      ...
      CM_SELECTSERVER: begin
        if not puser.SelServerOk then  //한번만 선택, (해커공격방지)
...
// remote public address를 입력하면, Gate 중 한곳이 리턴된다.
function GetUserGateAddr(svname, addr: string; var port: integer): string;
var
  i, j, mg, n: integer;
