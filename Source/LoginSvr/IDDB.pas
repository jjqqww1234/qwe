...
type
   {TUserInfo = record
      ...
      ServerIndex: integer;  //최근 선택한 서버 인덱스
      PasswdFail: integer;  //비밀번호 틀림횟수
      PasswdFailTime: longword; //비밀번호를 마지막으로 틀린 시간
      Memo: string[20];
      Demmy: array[0..9] of byte;
   end;}

  TUserInfo = record      //새로 변경 되는 사용자 정보 레코드
    UInfo: TUserEntryInfo;
  ...
  FIdRcd = record              //새로 변경된 계정 레크도
    Deleted: boolean;          {delete mark}
    MakeRcdDateTime: TDouble;  //계정이 만들어진 시간
    UpdateDateTime: TDouble;   //TDateTime;
    Key:     string[10];
    Block:   TUserInfo;
  end;
  ...
  //인덱스 파일의 마지막으로 고친 레코드와 데이터 파일의 마지막으로 고친 레코드를 비교,
  //인덱스 파일의 마지막으로 고친 레코드의 실제 데이터 파일의 시간을 검사 하여
  //유효한지를 검사한다.
            ...
            for i:=0 to header.MaxCount-1 do begin
               ...
               if FileRead (fhandle, rcd, sizeof(FIdRcd)) = sizeof(FIdRcd) then begin
                  //if not rcd.Deleted then begin
                     ...
                     newrcd.MakeRcdDateTime := PackDouble(Now); //계정이 만들어진 시간
...
begin
  ...
  if FileSeek(fhandle, rpos, 0) = rpos then begin
    ...
    if asnew then begin //추가
      ...
      rcd.MakeRcdDateTime := PackDouble(nowtime); //계정이 만들어진 시간
    end;
    rcd.Deleted := False;
...
begin
  ...
  if WriteRecord(integer(IdIndex.Objects[index]), rcd, False) then begin
    Result := True;
    ///IdIndex[index] := rcd.Key; <= 키는 바뀔 수 없다.
      {if flag then
